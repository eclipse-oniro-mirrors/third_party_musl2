...
static inline struct symdef find_sym2(struct dso *dso, const char *s, int need_def, int use_deps)
{
	...
	for (; dso; dso=use_deps ? *deps++ : dso->syms_next) {
		...
		def.sym = sym;
		def.dso = dso;
		//DEBUG(FIND_SYM, "Symbol: %s found in %s\n", s, dso->name);
		break;
	}
...
{
	if (needed_by) {
		INFO(LOAD_LIB, "Load library: %s needed by %s (%p)\n", name, needed_by->name, needed_by->base);
	} else {
		INFO(LOAD_LIB, "Load library: %s \n", name);
	}
	...
	if (is_self) {
		DEBUG(LOAD_LIB, "Catch load ldso itself, addr: %p\n", ldso.base);
		if (!ldso.prev) {
			tail->next = &ldso;
			ldso.prev = tail;
			tail = &ldso;
	...
	if (strchr(name, '/')) {
		...
		for (p=head->next; p; p=p->next) {
			if (p->shortname && !strcmp(p->shortname, name)) {
				DEBUG(LOAD_LIB, "Library: %s(%p) was already loaded\n", name, p->base);
				return p;
			}
		}
		...
		fd = -1;
		if (env_path){ 
			DEBUG(LOAD_LIB, "Load library: %s from LD_LIBRARY_PATH: %s\n", name, env_path);
		...
		for (p=needed_by; fd == -1 && p; p=p->needed_by) {
			...
			if (p->rpath){
				DEBUG(LOAD_LIB, "Load library: %s from rpath: %s\n", name, p->rpath);
		...
		if (fd == -1) {
			...
			DEBUG(LOAD_LIB, "Load library: %s for system path: %s\n", name, sys_path);
		}
		pathname = buf;
	}
	if (fd < 0) return 0;
	...
	for (p=head->next; p; p=p->next) {
		if (p->dev == st.st_dev && p->ino == st.st_ino) {
			...
			DEBUG(LOAD_LIB, "Found same so file with different name: %s(%p)\n", p->shortname, p->base);
			close(fd);
			return p;
		}
	}
	...
	DEBUG(LOAD_LIB, "name: %s, pathname: %s (%p)\n", name, pathname, p->base);
	return p;
}
...
{
	INFO(DL_OPEN, "Start dlopen file: %s\n", file);
	...
	if (!p->relocated || (mode & RTLD_GLOBAL)) {
		...
		DEBUG(DL_OPEN, "Add so: %s(%p) into symbol chain\n", p->name, p->base);
		add_syms(p);
		for (i=0; p->deps[i]; i++) {
			DEBUG(DL_OPEN, "Add so: %s(%p) into symbol chain\n", p->deps[i]->name, p->deps[i]->base);
			add_syms(p->deps[i]);
		}
	}
	if (!p->relocated) {
	...
	if (!(mode & RTLD_GLOBAL)){
		DEBUG(DL_OPEN, "Revert symbol chains %s(%p)", orig_syms_tail->name, orig_syms_tail->base);
		revert_syms(orig_syms_tail);
	}
